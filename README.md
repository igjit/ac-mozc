...
半角英数字の直後に日本語を入力したい、例えば `Emacs拡張` と入力したい場合は、`Emacs kakucyou` のように間にスペースを1つ入れてください。変換を実行するとスペースが削除されます。undoすると再びスペースが挿入されます。変換実行時にスペースを自動で削除してほしくない場合は、設定項目 `ac-mozc-remove-space` を `nil` に設定してください。
